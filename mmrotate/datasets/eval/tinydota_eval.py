...
class TinyDOTAeval:
    def __init__(self, annotations=None, results=None, numCats=9, iouType='mAP', nproc=10):
        ...
        self.params = TinyDOTAParams(iouType=iouType)  # parameters
    ...
    def evaluate(self, device='cuda:0'):
        ...
        if D > 0 and G > 0:
            ...
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    dtIg[tind, dind] = gtIg[gind]  # 匹配到ignore的gt，则dt也ignore

        # set unmatched detections outside of area range to ignore (逻辑不变)
...
def init_worker_with_counter(counter, devices_list):
    ...
    Uses a shared counter to get a unique ID and assign a device.
    The counter itself is process-safe.
    """
    global worker_device
